# -*- coding: utf-8 -*-
"""
Uplifting followed by prolongation interface (UPI) approach for ROM-FOM coupling
...
def export_data_UPI(nx,ny,n,w,s,t):
    folder = 'data_'+ str(nx) + '_' + str(ny)       
    if not os.path.exists('./UPI/'+folder):
        os.makedirs('./UPI/'+folder) 
...
if training == 'true':
    ...
    for i in range(nstart,nend,nstep):
        ...
        tmp = w.reshape([-1,])-wm
        aF = PODproj_svd(tmp,Phiw[:,:nr])
        tmp = t.reshape([-1,])-tm
        bF = PODproj_svd(tmp,Phit[:,:nr])    
...
aUPI = np.zeros([ns+1,nq])
bUPI = np.zeros([ns+1,nq])

tmp = w.reshape([-1,])-wm
aUPI[0,:] = PODproj_svd(tmp,Phiw)

tmp = t.reshape([-1,])-tm
bUPI[0,:] = PODproj_svd(tmp,Phit)

time=0
xtest = np.zeros((1,lookback,features.shape[1]))   
tmp = np.copy(aUPI[0,:nr])
...
for i in range(lookback-1,ns):
    ...
    w = PODrec_svd(aUPI[i+1,:],Phiw) + wm
    s = PODrec_svd(aUPI[i+1,:],Phis) + sm
    ...
    if (i+1)%freq==0:
        export_data_UPI(nx,ny,nstart+i+1,w,s,t)
...
tmp = t.reshape([-1,])-tm
bUPI[i+1,:] = PODproj_svd(tmp,Phit)

#%% Save DATA
folder = 'data_'+ str(nx) + '_' + str(ny)       
filename = './UPI/'+folder+'/UPI_data_nr='+str(nr)+'.npz'
np.savez(filename, aUPI = aUPI, bUPI = bUPI) 
